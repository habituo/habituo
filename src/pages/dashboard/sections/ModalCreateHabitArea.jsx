...
const ModalCreateHabitArea = ({ isOpen, onClose, selectedHabit }) => {
  ...
  const handleSave = async () => {
    if (!validateName(habitName) || !user) return;
    ...
    try {
      if (selectedHabit) {
        const habitRef = doc(
          db,
          `users/${user.uid}/areas/${selectedArea}/habits/${selectedHabit.id}`
        );
        ...
        await addDoc(
          collection(db, `users/${user.uid}/areas/${selectedArea}/habits`),
          {
            name: habitName,
            icon: selectedIcon,
            goal: { value: goalValue, unit: goalUnit, period: goalPeriod },
      ...
      toast({
        ...
        description: "No se pudo agregar el hábito. Inténtalo de nuevo.",
        status: "error",
        duration: 3000,
        isClosable: true,
        position: "bottom-center",
